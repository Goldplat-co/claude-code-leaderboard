...
#
#   # (b) 인자 없이 실행 → 터미널에서 닉네임 입력 받기
#   curl -sL https://claude-code-leaderboard-blue.vercel.app/install.sh | bash
#
# 환경변수 (선택):
#   PROJECT_DIRS  콜론 구분 프로젝트 루트. 미지정 시 자동 탐지.
#                 예) PROJECT_DIRS="$HOME/work/foo:$HOME/work/bar"
# ──────────────────────────────────────────────────────
...
# ── 1. 닉네임 확인 (인자 없으면 기존값 사용 또는 입력) ──
if [[ -z "$NICKNAME" ]]; then
  if [[ -f "$CONFIG_FILE" ]]; then
    NICKNAME=$(python3 -c "import json; print(json.load(open('$CONFIG_FILE')).get('nickname',''))" 2>/dev/null || echo "")
    if [[ -n "$NICKNAME" ]]; then
      echo "👤 기존 닉네임 사용: $NICKNAME"
    fi
  fi
fi
if [[ -z "$NICKNAME" ]]; then
  read -r -p "👤 리더보드에 표시할 닉네임: " NICKNAME
  NICKNAME="${NICKNAME#"${NICKNAME%%[![:space:]]*}"}"
  NICKNAME="${NICKNAME%"${NICKNAME##*[![:space:]]}"}"
fi
if [[ -z "$NICKNAME" ]]; then
...
echo ""
echo "--- 대시보드 (환경 스냅샷) ---"
PROJECT_DIRS="$PROJECT_DIRS" "$CONFIG_DIR/snapshot.sh" send 2>&1 || echo "   ⚠️  대시보드 전송 실패"
